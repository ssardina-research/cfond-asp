...
from cfondasp.base.elements import Action, FONDProblem, State, Variable
...
class AcrobaticsKnowledge(object):
    ...
    def __init__(
        self, fond_problem: FONDProblem, variables: List[Variable], output_dir: str
    ) -> None:
    ...
    def add_weakplan_knowledge(self):
        ...
        line = f":- state(t), holds(State, {var_idx}, {0}).\n"
    ...
    def add_control_knowledge(self):
        ...
        for _i in range(len(sorted_positions) - 1):
            ...
            line = f"adjacent({positions[i]}, {positions[j]}).\n"
            constraints.append(line)

            line = f"adjacent({positions[j]}, {positions[i]}).\n"
            constraints.append(line)

            line = f"adjacent({positions[i]}, {positions[i]}).\n"
            constraints.append(line)

        # :- holds(S1, 1, X), not goalState(S1), {holds(S2, 1, Y): successor(S1, S2), adjacent(X, Y)} =0.
        line = f":- holds(S1, {position_var_idx}, X), not goalState(S1), {{holds(S2, {position_var_idx}, Y): successor(S1, S2), adjacent(X, Y)}}=0. \n"
        constraints.append(line)
        ...
        # :- state(State), holds(State, 0, 0).
        line = f":- state(State), holds(State, {var_idx}, {0}).\n"
        constraints.append(line)
