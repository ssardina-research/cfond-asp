...
def write_variables(file, variables: List[Variable]):
    with open(file, "w+") as f:
        f.write(f"{ASP_VARIABLE_TERM}(0..{len(variables) - 1}).{os.linesep}")
        for var_id in range(len(variables)):
            var = variables[var_id]
            f.write(f"{ASP_VARIABLE_ATOM_TERM}({var_id}, 0..{len(var.domain) - 1}).{os.linesep}")
        f.write(os.linesep)
...
def write_mutex(file, mutexs: List[State]):
    ...
    with open(file, "a") as f:
        for idx in range(len(mutexs)):
            ...
            if count > 1:
                counter += 1
                f.write(f"{ASP_MUTEX_GROUP_TERM}({counter}).{os.linesep}")
                for var_id in range(len(state.variables)):
                    val = state.values[var_id]
                    if val != -1:
                        f.write(f"{ASP_MUTEX_TERM}({counter}, {var_id}, {val}).{os.linesep}")
        ...
        for var_id in range(len(variables)):
            ...
            for i in range(len(variable.domain)):
                f.write(f"{ASP_MUTEX_TERM}({counter}, {var_id}, {i}).{os.linesep}")
        f.write(os.linesep)
...
def write_initial_state_positive(file, state: State):
    with open(file, "a") as f:
        for var_idx, var in enumerate(state.variables):
            ...
            f.write(f"{ASP_HOLDS_TERM}(X, {var_idx}, {val}):- {ASP_INITIAL_STATE_TERM}(X).{os.linesep}")
    ...
    with open(file, "a") as f:
        for var_idx, var in enumerate(state.variables):
            ...
            for other_val in other_values:
                f.write(f"-{ASP_HOLDS_TERM}(X, {var_idx}, {other_val}):- {ASP_INITIAL_STATE_TERM}(X).{os.linesep}")
...
def write_goal_state(file, state: State):
    with open(file, "a") as f:
        for var_idx, var in enumerate(state.variables):
            val = state.values[var_idx]
            if val != -1:
                f.write(f"{ASP_HOLDS_TERM}(X, {var_idx}, {val}):- {ASP_GOAL_STATE_TERM}(X).{os.linesep}")
    ...
    with open(file, "a") as f:
        for action_name, det_actions in nd_actions.items():
            ...
            if action_type not in action_types:
                ...
                f.write(f'{ASP_ACTION_TYPE_TERM}("{action_type}"). {os.linesep}')

            # associate action type with action
            first_action: Action = det_actions[0]
            _name = get_ndet_action_string(first_action)

            f.write(f'{ASP_ACTION_TYPE_TERM}("{action_type}", "{_name}"). {os.linesep}')
            f.write(f'{ASP_ACTION_TERM}("{_name}"). {os.linesep}')
            ...
            for var_idx, var in enumerate(prec.variables):
                ...
                if val != -1:
                    f.write(f'{ASP_PREC_TERM}("{_name}", {var_idx}, {val}). {os.linesep}')

            # effects
            num_effects = len(det_actions)
            f.write(f'{ASP_EFFECT_COUNT_TERM}("{_name}", {num_effects}). {os.linesep}')
            ...
            for i in range(num_effects):
                ...
                f.write(f'{ASP_ACTION_EFFECT_TERM}("{_name}", "{effect}"). {os.linesep}')
                ...
                for var_idx in range(len(action.add)):
                    ...
                    if val != -1:
                        ...
                        f.write(f'{ASP_ADD_TERM}("{_name}", "{effect}", {var_idx}, {val}). {os.linesep}')
                ...
                for var_idx in range(len(action.delete)):
                    ...
                    if len(vals) > 0:
                        for val in vals:
                            affected_vars.add(var_idx)
                            f.write(f'{ASP_DEL_TERM}("{_name}", "{effect}", {var_idx}, {val}). {os.linesep}')

                # affects
                if variable_mapping:
                    # for var_idx in affected_vars:
                    #     # f.write(f'{ASP_AFFECTS_TERM}("{_name}", "{effect}",  {var_idx}). {os.linesep}')
                    unaffected_vars = total_variables.difference(affected_vars)
                    for var_idx in unaffected_vars:
                        f.write(f'not{ASP_AFFECTS_TERM}("{_name}","{effect}", {var_idx}). {os.linesep}')

        if precedence and max_nd_effect > 1:
            for i in range(1, max_nd_effect):
                f.write(f'precedence("{ASP_EFFECT_TERM}{i}", "{ASP_EFFECT_TERM}{i+1}"). {os.linesep}')

        f.write(f"{ASP_NDSIZE_TERM}({max_nd_effect}).{os.linesep}")
        f.write(os.linesep)
        ...
        for _nd_actions in nd_actions.values():
            ...
            if len(_nd_actions) > 0:
                ...
                for (i, j) in pairs:
                    ...
                    f.write(f"{ASP_SIBLING_TERM}({_name_1}, {_name_2}). {os.linesep}")
                    f.write(f"{ASP_SIBLING_TERM}({_name_2}, {_name_1}). {os.linesep}")

                    if precedence:
                        f.write(f"precedence({_name_1}, {_name_2}). {os.linesep}")

            if _nd_actions[0].prefix_name not in num_effects:
                count = len(_nd_actions)
                num_effects[_nd_actions[0].prefix_name] = count
                f.write(f'{ASP_EFFECT_COUNT_TERM}("{_nd_actions[0].prefix_name}", {count}).{os.linesep}')
...
def write_undo_actions(clingo_output_file: str, grounded_undo_file: str, process_action_type=False):
    ...
    for [a1, a2] in undo_actions:
        if process_action_type and action_type(a1) != action_type(a2):
            ...
            line = f':- {{policy(S2, "{a2}"): policy(S1, "{a1}"), transition(S1,"e1", S2)}}!=0.{os.linesep}'
    ...
    for a1_type, a2_type in undo_action_types.items():
        line = f':- {{policy(S2, A2): policy(S1, A1), actionType("{a1_type}", A2), actionType("{a2_type}", A1), transition(S1,"e1", S2)}}!=0.{os.linesep}'
        constraints.append(line)
